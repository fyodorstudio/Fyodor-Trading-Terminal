...
`FMS Experiment Workbench` is the active repeatable research surface; `Differential Calculator` remains an active experiment. Event Replay, Macro Drivers, and Prototyping are retained under Garbage / Ignore. Charts exposes `FMS-REGISTERED-REACTION-H4-v4`: 47 immutable recipes across EURUSD, GBPUSD, USDJPY, AUDUSD, USDCAD, NZDUSD, and USDCHF. Every registration reconciles with its current scoring-engine experiment and chronological later-test audit; marginal positive histories remain visibly labelled `Positive but fragile`. Historical matches are explicitly hindsight; Current mode uses only immutable post-activation release observations. `FMS Shadow Trader` is global across registered markets and separates whether price moved in the registered direction from whether the exact SL/TP/duration contract captured that move. Failed backtests remain explicit contender or avoid-directional-use knowledge instead of being discarded. Nothing is a guarantee or automatic order.
