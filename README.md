...
`FMS Experiment Workbench` is the active repeatable research surface; `Differential Calculator` remains an active experiment. Event Replay, Macro Drivers, and Prototyping are retained under Garbage / Ignore. Charts exposes `FMS-HISTORICALLY-PROFITABLE-H4-v1` for EURUSD, GBPUSD, USDCAD, and USDJPY. It preserves registered EURUSD rules and adds deduplicated positive walk-forward recipes discovered by the seven-market sweep. Historical matches are explicitly hindsight; Current mode uses only immutable post-activation release observations. `FMS Shadow Trader` is global across registered markets: it shows every monitored rule, current hypothetical state, nearest upcoming registered release, sortable profitability/target-first history, and gross sequential replay. It also retains failed backtests as explicit contender or avoid-directional-use knowledge instead of discarding them. Nothing is a guarantee or automatic order.
