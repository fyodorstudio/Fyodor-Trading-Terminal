...
`FMS Experiment Workbench` is the active repeatable research surface; `Differential Calculator` remains an active experiment. Event Replay, Macro Drivers, and Prototyping are retained under Garbage / Ignore. Charts exposes `FMS-REGISTERED-REACTION-H4-v5`: the 47 immutable v4 recipes plus three explicitly reviewed break-even execution upgrades for AUDUSD US producer inflation, NZDUSD US producer inflation, and USDJPY Japan inflation. Every registration reconciles with its current scoring-engine experiment and chronological later-test audit; marginal positive histories remain visibly labelled `Positive but fragile`. Historical matches preserve the contract active at their event time; Current mode uses only immutable post-activation release observations. `FMS Shadow Trader` is global across registered markets and separates whether price moved in the registered direction from whether the exact execution contract captured that move. Failed backtests remain explicit contender or avoid-directional-use knowledge instead of being discarded. Nothing is a guarantee or automatic order.
