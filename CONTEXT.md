...
- Source-clock alignment requires empirical provenance, not just SDK documentation. On 15 Sep 2026 the owner's live BTCUSD sample showed native MQL and Python M1/H4 candle timestamps agree and both are approximately +3h from UTC receipt time. Calendar/candle geometry must not be shifted by that offset; comparing UTC receipt/decision clocks directly with native release/entry clocks is unsafe. New observations retain immutable `sourceClock` metadata from explicit contemporaneous EA GMT/estimated-server samples. Before a new current first decision, the bridge also requires one bounded broker-catalog/day-scoped Python MT5 candle calibration; unavailable, stale, mismatched, uncertain or transitioning evidence fails closed. Existing decisions are preserved and older receipts are never backfilled. Historical archive comparability/DST remains unverified. Never apply today's offset globally to history or let a user-selected display timezone control financial decisions.
...
Charts left panel has four windows: **Trade** owns Next/Current/Recent; **Journal** owns longer performance records; **Setups** owns registered benchmarks plus lazy Research/Knowledge disclosures; **Past Result** owns the selected arrow audit. Past Result is one plain `Field / Value / Details` table, ordered from frozen result to initial reaction and then supporting evidence; entry-timing rows appear only when immutable first-seen quote data exists. Trade view/disclosure/filter/search/scroll state and Setups disclosure state are session-persisted. Trade rows with a recorded signal can switch to the owning pair/entry timeframe, reveal that setup, focus its activation arrow, and open Past Result.

FMS evidence ownership is intentionally layered. Offline entry/execution research reads frozen manifests/cached candles and never imports the live server or MetaTrader5. `registered_entry_review_evidence.json` is the single reviewed-H1 runtime record. `registered_execution_successors.json` is the hash-validated FMS v2 execution registry: its code-owned allowlist has 17 profiles, activation `1789896633`, and cannot auto-promote research rows. Both have fail-closed loaders and preserve prior contracts. `fms_historical_evidence.py` owns canonical chart evidence schema/source priority; the bridge only adapts storage. `chartMacroBiasAuditViewModel.ts` owns Past Result interpretation while the React component only renders its table rows.
...
The direction-first event-respect campaign is separately versioned from the registered trading release. Corrected campaign v4 is frozen through chronological challenge: 9 declared development candidates produced 0 challenge-supported, 7 prospective-only, and 2 rejected rows. Prospective observation began at Unix `1789439876` and records only immutable first-seen post-activation releases; it has no SL, TP, order action, or automatic promotion. This campaign's R5 execution/high-TP stage remains blocked pending enough new evidence. The frozen baseline has the readable label `FMS v1`; its actual model IDs, contracts, activation boundaries and results are unchanged. `FMS v2` now identifies only the 17 explicitly approved event-specific execution successors activated at `1789896633`; the other 34 recipes and every earlier arrow remain v1. This is not the provisional campaign and does not change that campaign's gate. Campaign calculations live in pure `fms_reaction_campaign.py`; staged immutable artifacts are created by `scripts/run_event_respect_campaign.py`, and the Workbench reads them through `/research/event-respect-campaign`. [FMS v2 Research](docs/Development%20Logs/FMS%20v2%20Research.md) records successor direction and source evidence.
