...
- Source-clock alignment is an explicit research gate: the EA forwards raw trade-server calendar timestamps, while Python MT5 documents candle times as UTC. Stored completion/receipt clocks differ by roughly three hours, but historical release/candle mapping and DST offsets remain unverified. The v2 reference catalogue is therefore as-recorded descriptive evidence, not verified post-information execution. Preserve originals; never apply today's offset globally to history or approve successor timing before verifying period-specific alignment.
...
FMS evidence ownership is intentionally layered. Offline entry research reads frozen manifests/cached candles and never imports the live server or MetaTrader5. `registered_entry_review_evidence.json` is the single reviewed-H1 runtime record, guarded by a non-promoting fixed-allowlist publisher and fail-closed loader. `fms_historical_evidence.py` owns canonical chart evidence schema/source priority; the bridge only adapts storage. `chartMacroBiasAuditViewModel.ts` owns Past Result interpretation while the React component only renders its table rows.
