...
export const ChartFmsActionCard = memo(function ChartFmsActionCard({ data }: { data: ChartMacroBiasRealtimeCardData }) {
  const markets = data.globalResponse?.markets.filter((market) => market.supported) ?? [data.response];
  ...
  const action = globalBlock
    ...
    : primary == null
      ...
      : primary.signal.entry == null
        ? { state: "WAITING", title: "Wait for the H4 entry", detail: `The frozen entry is ${formatUtcDisplayDateTime(activation!)}. Do not enter before it.`, tone: "waiting" }
        : withinEntryGrace
      ...
      {primary ? (
        <>
          ...
          <dl className="fms-action-grid">
            <div><dt>Release</dt><dd>{formatUtcDisplayDateTime(primary.signal.eventTime)}</dd></div>
            <div><dt>Frozen entry</dt><dd>{activation == null ? "Unavailable" : formatUtcDisplayDateTime(activation)}</dd></div>
            ...
            <div><dt>Expiry</dt><dd>{primary.signal.expiryTime ? formatUtcDisplayDateTime(primary.signal.expiryTime) : `${primary.signal.expiryCandles} H4 candles`}</dd></div>
