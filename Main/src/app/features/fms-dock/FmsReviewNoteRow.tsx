...
import { formatChartUtcMetadataTime, type ChartDisplayTimeMode } from "@/app/lib/chartView";
  ...
  return <tr className="fms-review-note-row" data-note-record-key={input.recordKey}>
    ...
    <td colSpan={valueColSpan}>
      {editing ? <form onSubmit={(event) => onSave(event, { ...input, label })}>
        ...
        <span><time dateTime={new Date(saved.updatedAt * 1000).toISOString()}>Updated {formatChartUtcMetadataTime(saved.updatedAt, displayTimeMode, sourceTimeOffsetSeconds)}</time><button type="button" onClick={onEdit}>Edit</button><button type="button" disabled={saving} onClick={onRemove}>{saving ? "Removing..." : "Remove"}</button></span>
      </div> : <div>
