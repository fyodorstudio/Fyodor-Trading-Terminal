...
`Main/src/styles.css` used to be a CSS monolith. It is now an ordered active-style import aggregator.
...
- `Main/src/styles.css` must stay as the active import aggregator.
- `Main/src/styles/garbage.css` is the garbage-only import aggregator and is loaded by garbage routes, not by default active app startup.
...
- `02-garbage-legacy-terminal-overview.css` - old terminal/narrative/overview experiments retained for garbage compatibility.
- `03a-garbage-legacy-overview-brief.css` - old unused overview brief / decision / story selectors.
...
- `09-garbage-event-quality-reaction-study.css` - garbage Archived Event Quality / Event Reaction continuation styles.
...
- keep active Charts work in `15-charts.css`;
- keep garbage route styling inside the files imported by `garbage.css`;
...
- keep active import order equivalent after each move;
