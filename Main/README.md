...
- `Macro Signal Lab` is a EURUSD/H4-only immutable-version research experiment backed by durable MT5 calendar history. It preserves rejected Economy v1 and active country-aware Labor v2. Charts has an opt-in `Macro bias` layer that shows only recurring v2 patterns passing fixed historical development-and-holdout gates. These arrows are experimental research classifications, never automatic orders or guaranteed outcomes; the forward ledger independently monitors what happens after registration.
