...
- `FMS Experiment Workbench` records immutable H4 economic-event experiments across the cached major-pair markets. Charts owns `FMS-REGISTERED-REACTION-H4-v3`: 16 pair-orientation-audited recipes across EURUSD, GBPUSD, USDJPY, AUDUSD, USDCAD, and NZDUSD expose positive gross no-lookahead walk-forward average R with frozen scoring, case filter, reaction, SL, TP, and expiry. Workbench also exposes the pair-specific Reaction Atlas. Confidence, stability, and omitted costs remain visible diagnostics, and Shadow Trader explicitly states that no setup is safe to follow blindly.
...
- All chart timeframes show the same frozen H4 model. Intraday views connect release rings to strictly later H4 arrows when representable; coarse views retain the activation arrow without inventing intrabar detail.
