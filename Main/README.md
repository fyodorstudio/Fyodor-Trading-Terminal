...
- `FMS Experiment Workbench` records immutable H4 economic-event experiments across the cached major-pair markets. Charts owns `FMS-REGISTERED-REACTION-H4-v5`: the 47 pair-orientation-audited v4 recipes plus three explicitly reviewed break-even execution upgrades. Historical arrows preserve the execution rule active at their event time; future matching uses the reviewed rule only after its activation boundary. Workbench also exposes the pair-specific Reaction Atlas. Arrow audits separate the six-H4 evidence reaction from the final frozen trade result, including MFE, MAE, giveback, and fixed-horizon responses. Confidence, stability, fragile-history labels, omitted costs, and the absence of live validation remain visible diagnostics, and Shadow Trader explicitly states that no setup is safe to follow blindly.
...
- Shadow Trader's practical-registry forward lifecycle is backed by append-only first-seen decisions. Successful EA cycles advance pending candle-path paper trades, while packages first observed at or after their frozen H4 entry remain audit-only and cannot create prospective trades. The UI treats this as ready for demo-only monitoring; forward sample breadth, fills, costs, and strict account-risk qualification remain optional diagnostics rather than blockers. Real-money execution stays out of scope.
- When Macro Bias is open, the global Shadow Trader refreshes silently every 30 seconds across all seven registered markets. It discovers new qualified/no-trade packages without requiring an existing pending trade or a page reload, while preserving the last response during transient bridge failures.
- Qualified prospective trades include an optional deterministic `FMS-…` comment tag. A manually placed MT5 demo trade carrying that exact tag is reconciled read-only from deal history, including actual price, profit, commission, swap, and fee; direction, SL, and TP are checked against the frozen contract, non-demo accounts are blocked, and Fyodor never transmits an order.
