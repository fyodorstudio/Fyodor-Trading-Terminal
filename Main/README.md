...
- `Macro Signal Lab` is a EURUSD/H4 immutable-version research experiment backed by durable MT5 calendar history. It preserves rejected Economy v1 plus country-aware Labor v2, Sentiment v3, Policy/Inflation v5, and Growth v7. Charts owns prospective `FMS-EURUSD-FORECAST-GUARD-H4-v13`: `Current model` uses post-activation immutable releases matching the retained Euro-area-consumer-sentiment, US-industrial-output, exact US-payroll, or complete US-producer-inflation-cooling setup identities. V13 preserves raw MT5 values but excludes a Forecast comparison when a past-only exact-series detector identifies an anomalous Forecast/Previous gap; Momentum remains eligible and no agreement bonus is invented. `Research replay` remains the historical research view. Payroll and producer inflation retain their frozen stop/target/expiry contracts. The `FMS Shadow Trader` reports Long/Short/No-trade, why, a configurable gross-only sequential account replay, the hypothetical ATR position, an MT5-clock countdown and if/then plan for the next setup, registered benchmarks, and descriptive policy/inflation context. It permits one simulated position at a time and excludes spread, commission, slippage, and swap rather than estimating them. Current data begins preloading at app startup, independently of the Macro Bias control, and is projected across every EURUSD chart timeframe as the same frozen H4 model. Intraday views connect release rings to strictly later H4 arrows when the timestamps occupy different displayed candles; coarse views retain the activation arrow without inventing intrabar separation. Nothing executes an order or guarantees an outcome.
- Macro Signal Lab's Path and Exit Stress Research loads independently and audits full-path MFE/MAE plus a fixed development-selected stop/target/holding matrix. Charts defaults to Current Model and can separately show backward `Historical matches` of those exact current rules; neither surface relabels reused history as a live signal.
- Macro Signal Lab's separate v11/v12 reports split numeric history by S/M agreement, revision reliability, package completeness, Before alignment, score magnitude, continuation/rejection, and Forecast policy. They remain research surfaces; v13 promotes only the corrected Forecast-quality input safeguard, not the failed new setup candidates.
