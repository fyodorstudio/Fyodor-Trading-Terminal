...
- `FMS Experiment Workbench` records immutable H4 economic-event experiments across the cached major-pair markets. Charts owns `FMS-HISTORICALLY-PROFITABLE-H4-v1`: EURUSD, GBPUSD, USDCAD, and USDJPY expose deduplicated recipes with positive no-lookahead walk-forward average R and frozen per-setup scoring/SL/TP/expiry. Confidence, stability, and omitted costs remain visible diagnostics instead of silently suppressing the practical registry.
