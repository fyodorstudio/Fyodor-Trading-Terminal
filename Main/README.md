...
- `FMS Experiment Workbench` records immutable H4 economic-event experiments across the cached major-pair markets. Charts owns `FMS-REGISTERED-REACTION-H4-v2`: five pair-orientation-audited EURUSD, GBPUSD, and USDJPY recipes expose positive gross no-lookahead walk-forward average R with frozen scoring/SL/TP/expiry. Confidence, stability, and omitted costs remain visible diagnostics, and Shadow Trader explicitly states that no setup is safe to follow blindly.
