...
**2026-09-01 observed demo-execution comparison:** Shadow Trader now compares explicitly tagged MT5 demo deals with the immutable FMS candle-path plan. It reports actual entry delay, direction-adjusted entry difference in R, gross fill result versus the frozen result, recorded commission/swap/fee impact, and contract adherence. The readiness report uses the same durable demo ledger instead of claiming zero observed fills. Entry difference deliberately remains a combined observation: MT5 deal history cannot reliably separate manual delay, spread, slippage, and intervening price movement. A separate operational preflight now blocks a new demo decision when the successful EA calendar cycle is missing/stale or its latest cycle reported failed batches. Setup-specific prospective evidence is frozen as `FMS-SETUP-FORWARD-GATE-v1`: at least 10 resolved cases, 90 elapsed days, positive average R, and 80% near-entry quote coverage. A setup with at least 10 resolved cases and non-positive average R immediately loses prospective-supportive status and is labelled `Needs review`; its immutable historical registration is preserved for audit. An advisory manual limited-live review state additionally requires five completed contract-adherent tagged demo trades with positive average net R for the exact setup, the existing 30-trade manual-demo risk-policy audit, and a current operational preflight. This state is evidence for a later human review only: Fyodor still sends no order and the real-money gate remains closed.
...
**2026-08-30 registered reaction-profile pass:** all 47 registered recipes now own a durable chronological later-test reaction profile at fixed `1/3/6/12/30 H4` horizons. Each horizon reports alignment and direction-adjusted pips/ATR/R distributions; the path audit reports MFE, MAE, time to each extreme, and giveback. A deterministic label distinguishes continuation, short-lived impulse, delayed continuation, initial rejection, volatility-only, and no dependable reaction. Arrow audits now state the first-H4 response before the immutable frozen trade result, so a release may truthfully read `Price followed the arrow` and later `SL reached -1R`. Shadow Trader also exposes a development-selected bounded contract comparison, but no alternative contract is promoted or allowed to rewrite an existing arrow.

The visually striking August EURUSD retail-sales examples did not generalize into dependable aggregate direction: the later sample is `N 28`, with alignment of approximately `53.6% / 46.4% / 42.9% / 35.7% / 53.6%` at `1/3/6/12/30 H4`, so the profile is classified `Volatility response only`. Its median best favorable path was still about `2.14 ATR` / `56.9 pips`, typically reached around 20 H4. This is precisely why FMS must keep reaction magnitude, directional repeatability, and frozen trade profitability as separate claims. Eight development-selected alternative contracts were retained as research diagnostics; none changed the active v4 registry in this pass.
...
#### Practical-registry forward lifecycle - 2026-08-31

The v5 practical registry now has its own append-only forward execution lifecycle. Each first-seen qualified decision remains immutable while later observations record Waiting-entry, Pending, TP, SL, break-even, Expired, Ambiguous, or Unavailable transitions. A successful EA cycle refreshes only markets with active paper cases from MT5 H4 candles and uses M1 when same-H4 boundary ordering requires it. A browser visit is not required to advance a case.

At the first available observation after the strictly later H4 entry, the bridge records MT5 bid/ask, quote time, spread in price/points, and capture lag. Quotes within 120 seconds of the model entry are labelled `near entry`; later observations are explicitly `late snapshot`. This is not represented as a broker fill, and commission, slippage, and swap remain unknown.

Shadow Trader exposes a frozen minimum paper gate of 50 resolved prospective cases, at least five individually paper-ready setups, positive aggregate average R, and at least 80% near-entry quote coverage. A setup becomes paper-ready only after at least ten resolved prospective cases of its own, positive average R, and at least 80% near-entry quote coverage. Passing that gate would support paper reliance only. The later manual-demo gate requires at least 30 completed exact-contract trades spread across at least five setups, with at least five completed and positive net average R for each represented ready setup. Real-money reliance remains `No` until those actual demo fills, account costs, risk limits, and the kill switch are verified. Historical results or candle-based paper outcomes can never make `Safe to follow blindly` become `Yes`.

Each qualified prospective trade also receives a deterministic short `FMS-…` tag. If the user manually places that exact direction on an MT5 **demo** account and pastes the tag into the order Comment, the bridge reads the resulting account history and stores its entry/exit deals, volume, prices, profit, commission, swap, and fee. Exit deals are linked through the MT5 position id even when the broker does not repeat the comment. The bridge also reads the associated MT5 position/order history and only counts a trade as contract-adherent when its direction, SL, and TP match the frozen FMS contract and its completed lifecycle agrees with the frozen model result within the declared fill/time tolerance. Reusing one tag for multiple positions is a duplicate-order violation. Non-demo accounts are rejected and the bridge contains no order-send path. This creates genuine execution evidence without authorizing automation or presenting a demo result as a real-money guarantee.

`FMS-MANUAL-DEMO-RISK-v1` is frozen before observing the tagged demo sample: maximum 0.25% actual initial account risk per trade, one open trade and 0.25% maximum portfolio risk, exact direction/SL/TP adherence, a pause at 5% peak-to-trough drawdown, and a pause after five consecutive losing trades. Initial risk is calculated through MT5 `order_calc_profit` from the actual demo fill to the actual recorded stop and stored with the account-balance snapshot. Any excessive risk, concurrent position, contract deviation, drawdown breach, or loss-streak breach pauses actionable Shadow Trader rows but preserves the underlying signal and audit. This is an operational research brake, not a claim that 0.25% risk makes a strategy safe.

#### Demo-first readiness correction - 2026-08-31

The near-term operating boundary is now demo-only Shadow Trader monitoring. Future-sample breadth, captured fills, execution costs, and `FMS-MANUAL-DEMO-RISK-v1` remain useful optional diagnostics, but they no longer veto a registered prospective demo signal. Real-money execution remains outside the product boundary.

Prospective integrity is the mandatory gate. The complete immutable release package and FMS decision must both exist before the setup's first strictly later H4 entry. A bridge restart may discover an already released package after that entry; such a package is permanently marked `late_for_contract`, retained for audit, excluded from forward statistics, and never displayed as an actionable current trade. Legacy decisions without proven first-seen timing default to ineligible rather than being trusted retroactively.

The visible operating surface now polls the global current registry every 30 seconds across all seven registered markets, regardless of whether a lifecycle was already pending. This is intentionally a discovery refresh: a newly ingested registered release can appear as No trade or Queued without reloading Charts, then advance to Open and a terminal result through the same response. Unchanged payloads do not replace React state, and transient bridge failures retain the last honest display.

#### Target frontier and prospective release quote - 2026-09-01

Every arrow now receives an independent target path ladder at `0.25/0.5/0.75/1/1.5/2/3/4R`, always measured against the original frozen SL and maximum duration. This answers whether a smaller target would have been reached before the original SL without changing the arrow's official result. Same-H4 target/SL collisions use cached M1 ordering when present and remain ambiguous when it is absent. These rows are independent full-position questions, not partial exits or realized profit.

The immutable `fms-execution-challenger-v2` artifact also contains a compact target frontier per recipe. It holds the active SL and duration fixed, selects one target using older development history, and reports its later result unchanged. The first audit surfaced promising target-only challengers—not promotions—including USDJPY US trade balance (`0.5R` active versus development-selected `4R`, later average approximately `+0.15R` versus `+0.41R`), USDJPY US manufacturing employment (`0.5R` versus `4R`, approximately `+0.10R` versus `+0.30R`), and EURUSD retail sales (`4R` versus `3R`, approximately `+0.08R` versus `+0.23R`). These reused-history observations still require the complete frozen review gate and cannot mutate the registry automatically.

For new registered releases processed within fifteen minutes of their scheduled time, FMS now records the first quote it actually observes after the release and stores bid, ask, quote time, spread, capture time, and release-to-observation lag with the immutable decision. This starts the prospective dataset needed to compare post-release entry with the existing later-H4 baseline. It does not claim the quote was the first market tick or an executable fill. Historical calendar rows lack immutable first-seen quote evidence and therefore cannot truthfully validate exact-release execution; they remain reaction research using the reproducible H4 baseline.

#### Prospective entry-timing audit - 2026-09-01

The forward observation record now also freezes the EA completed-package timestamp, bridge acknowledgement/first-seen timestamp, immutable FMS decision timestamp, and the first MT5 bid/ask tick retrievable after the complete package was observed. If tick history is unavailable, the source is explicitly retained as a later current snapshot rather than being relabelled as the first tick or a fill.
