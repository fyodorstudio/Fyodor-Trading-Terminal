...
## 2026-09-08 live release visibility fix

- Current includes today?s (Asia/Jakarta) released registered decisions without an open trade, alongside queued/open simulations. Preserve no-trade/recovered eligibility labels; move closed trades to Recent.
- Confirmed USDJPY labor-wages live endpoint returns short, late_for_contract, recovered_offline, pending, entry/activation null. This previously vanished because Current required entry/eligibility while Recent excluded pending outcomes.
- Selected-market refresh retries every 60 seconds after completion, reports failures visibly, and must not be replaced by older global snapshots. H1 path query excludes null activation times.
- Manual audit: USDJPY Current shows labor wages with no trade open, release calculations, and capture reason; switching pairs/reopening Charts retains visibility.

### FMS panel focused follow-up
- Deduplicate in-flight chart-signal requests; bound hung requests to 90 seconds. Reuse successful selected-market refresh for 60 seconds on quick pair switches, refresh immediately for changed calendar revision, preserve newer selected-market state.
- Current/Recent share one list; decision summary appears first and raw audit text is collapsed. Mixed-currency packages never get a single-currency score interpretation; flag forecast-guard exclusion.
- Measured cached global assembly: 0.22 seconds, 3.15 MB serialized. Browser rendering/pair-switch latency still requires owner manual verification; no claim of measured browser speedup.

## Charts and FMS dock robustness pass ? 2026-09-08

Implemented within Charts/left-panel scope:
- Global dock remains mounted while a chart pair loads; selected dock no longer resets to Trade on transient data changes. Panel render error includes Retry.
- All-market release monitor runs independently of selected pair; one due market at a time, request deduplication, per-market cooldown, visible monitoring errors separate from selected refresh errors. Current has a placeholder at scheduled release time pending evaluation.
- One partition owns Current/Recent: pending stays Current even with no entry or across midnight; today's no-trade decisions stay Current; completed trades and older no-trade decisions belong to Recent. Recent no longer silently caps at ten. Trade subtab persists in session storage.
- Shared FmsReleaseCards renders A/F/P/raw differences and frozen scoring in Trade and Past Result. Missing calculation values show unavailable rather than fabricated scores. Decision summary first; long audit and timing evidence collapsed; explicit capture reason.
- Setups no longer renders duplicate account/history/decision-ledger blocks; Journal owns results. Research/Setups hidden sections are not rendered for the other view. Knowledge evidence comparison is collapsible, clipboard errors are handled, sorting no longer mutates a shared list.
- Journal gross headline totals follow selected period; previous-week day strip follows previous week.
- Large bridge JSON uses gzip level 1: cached global sample 3.15 MB raw / 0.71 MB compressed; assembly measured 0.22 seconds.
Validation: TypeScript; direct lifecycle assertions for release handoff/null labels/missing-entry recovery/midnight/no-trade/completed/dedup; two TestClient global HTTP reloads preserved labor wages and gzip decoded; 50 server-render checks across 10 registered markets (flag images stubbed for SSR module compatibility). Existing Charts suite: 20 pass, 3 fail at older UI text assertions (arrow label, removed Initial move, Frozen contract heading). No new test files or browser automation.
Owner manual audit at 1440x900 / 100%: switch pairs during loading; reload with Current selected; open/close/reselect past arrow; inspect all docks; switch Journal periods; inspect next actual release transition and errors if bridge disconnects. Browser layout and latency remain unmeasured by agent.
