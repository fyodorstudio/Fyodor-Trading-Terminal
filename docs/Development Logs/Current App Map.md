...
3. `Charts`
   ...
   - The practical registry also writes an append-only prospective execution lifecycle after successful EA cycles. It advances paper trades without requiring Charts to be open and shows a compact demo-readiness surface in Shadow Trader. A release first captured at or after its frozen H4 entry is audit-only and cannot create a prospective trade. A package captured before entry is visibly queued with its planned strictly-later H4 time even though that future candle is not loaded yet, then becomes Open when the entry candle exists. Forward breadth, fills, costs, and the older strict risk policy remain optional diagnostics rather than blockers for demo-only monitoring; real-money execution remains out of scope.
