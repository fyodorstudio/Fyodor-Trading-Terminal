...
3. `Charts`
   ...
   - EURUSD has an opt-in `Macro bias` layer with `Current model` and `Research replay` views. Both project the frozen H4 model across the selected chart timeframe; they do not claim a separate lower- or higher-timeframe backtest. A gray dot owns the release candle when release and activation occupy different displayed candles; the directional arrow represents the first strictly later H4 open used by the simulation. Resolved current outcomes stop being active, and the audit exposes target sensitivity, execution stress, recent/year stability, uncertainty, and immutable model/data fingerprints.
   - Current `FMS-HISTORICALLY-PROFITABLE-H4-v1` is the practical Charts registry. It preserves three earlier EURUSD rules, replaces the older US-industrial-output rule with its positive walk-forward directional contract, and adds historically positive frozen recipes for GBPUSD, USDCAD, and USDJPY. The primary registration gate is immutable/no-lookahead construction plus positive pooled walk-forward average R with usable sample coverage; uncertainty, stability, concentration, and omitted trading costs remain prominent diagnostics rather than automatic academic vetoes. The top-right `FMS Shadow Trader` is global across all four registered markets: it reports every Long/Short/No-trade state, exact reason, nearest upcoming registered releases, a configurable gross-only account, one-position-at-a-time sequential replay, per-setup execution, and sortable historical profitability/target-first benchmarks. Arrow audits identify the actual market, distinguish a finite target/stop/expiry lifecycle from permanent direction, show the exact registered benchmark first, verify eight recipes against immutable experiments, and quarantine broader source diagnostics behind a separate disclosure; three preserved EURUSD rules remain visibly labelled legacy snapshots. A compact research-intelligence disclosure preserves promising-but-unstable contenders and repeatedly unsupported directional mappings as explicit avoid knowledge. Spread, commission, slippage, and swap are excluded rather than estimated; no order is sent to MT5. Every chart timeframe projects the frozen H4 model and must not imply a native lower/higher-timeframe backtest.
