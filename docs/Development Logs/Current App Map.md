...
3. `Charts`
   ...
   - Current `FMS-EURUSD-FORECAST-GUARD-H4-v13` prospectively retains the v10 setup identities and exits while adding a past-only exact-series Forecast-quality guard. Raw MT5 values remain visible; an anomalous Forecast contributes no Surprise point, Momentum remains eligible, and no agreement bonus is invented. The confirmed 2026-08-21 Consumer Confidence source error is shown as a Long audit reclassification but never retroactively opened because it predates v13 activation. Payroll uses `2 ATR / 1R / 6 H4`; producer inflation uses `2 ATR / 1.25R / 18 H4`; retained patterns otherwise keep their declared exits. The top-right `FMS Shadow Trader` reports Long/Short/No-trade, exact reason, a configurable gross-only account, one-position-at-a-time sequential replay, the per-setup hypothetical exit, an MT5-clock countdown and if/then plan, registered benchmarks, and factual EUR/USD policy/inflation context. Spread, commission, slippage, and swap are excluded rather than estimated; no order is sent to MT5. Current Model data is viewport-independent and reused across every EURUSD chart timeframe rather than reloaded during ordinary timeframe navigation. Every non-H4 view explicitly remains a projection of the frozen H4 model, not a native timeframe backtest.
