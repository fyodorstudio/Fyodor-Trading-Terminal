...
3. `Charts`
   - Current state: primary, useful surface.
   - Current override: `FMS-REGISTERED-REACTION-H4-v3` contains 16 immutable, pair-orientation-correct recipes across EURUSD, GBPUSD, USDJPY, AUDUSD, USDCAD, and NZDUSD. It supports continuation and rejection mappings plus declared past-only ordinary-magnitude filters. Every row reconciles with a current-scoring-engine experiment and walk-forward audit. USDCHF remains research-only because the atlas found no practical registered recipe. Shadow Trader explicitly says no setup is safe to follow blindly.
