...
3. `Charts`
   - Current state: primary, useful surface.
   - Current override: `FMS-REGISTERED-REACTION-H4-v2` replaces the older practical registry described below. It contains only pair-orientation-v2 recipes reconciled to new immutable experiments with positive gross later-test averages: EURUSD and GBPUSD US industrial output, plus USDJPY US consumer sentiment, Japan labor wages, and Japan headline/core inflation. Corrected USDCAD and USDJPY labor-claims failures were removed from live scanning. Shadow Trader explicitly says no setup is safe to follow blindly.
