...
- [ ] Pre-register and test the next FMS expansion without promoting archive-mined leads directly to arrows. Prioritize Euro-area producer inflation (failed only year share), then separately test US retail packages, small-sample EUR wages/US current account/US wage packages, and EUR GDP with fixed gates and explicit reused-history status.
