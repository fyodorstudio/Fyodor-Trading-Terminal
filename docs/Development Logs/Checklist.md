...
- [x] Add a dedicated `Journal` dock that groups every post-registration FMS decision by Asia/Jakarta day, totals resolved model R separately from actual tagged-demo dollar P/L, retains live/recovered/no-trade provenance, and shows entry/SL/TP/result plus any matching broker outcome.
- [x] Verify and display the connected MT5 demo account even while FMS is waiting for its first qualified live signal. Trade details expose the immutable demo tag that a manually placed order must carry for broker-history matching.
- [x] Keep order transmission unavailable. The bridge reads tagged demo deals and calculates their actual net result, but it cannot send or modify an MT5 order; reconstructed and historical cases never masquerade as broker executions.
...
- [x] Materialize `fms-market-context-v1` and `fms-context-challenger-v1` for all 47 registered recipes. Price trend, ATR regime, directional room to a repeatedly tested H4 pivot zone, Before evidence, and release session are calculated only from information available by entry. Each one-dimensional context is compared with its own unfiltered recipe on older development and later cases; the current arrow and frozen execution remain unchanged. Development-only selection produced 27 immutable candidates, of which 5 remained supported on untouched later history. Release session stays descriptive and cannot be selected because it may proxy event identity or daylight-saving changes.
- [x] Surface entry-known context before registration: Past FMS Result shows the exact selected context audit, and clicking an arrow draws the nearest confirmed H4 support/resistance barrier as a neutral amber chart line beside Entry/SL/TP. After code review, four supported rows moved into the separate context-conditioned registry; only the unregistered USDCAD retail context remains in `Needs Codex review`.
- [x] Preserve previously reviewed execution artifacts when refreshing context research. Context-only regeneration requires an explicit immutable base, preventing a revised candle fingerprint from silently deactivating or rewriting a reviewed contract.
- [x] Complete `FMS-CONTEXT-CONDITIONAL-H4-v1`: assign durable per-market `CTX-E` experiment identities, code-review five later-supported contexts, register four fingerprint-locked `CTX-C` rules, and retain USDCAD Canada retail sales as research-only because its 47.1% directional alignment did not support the arrow direction. A matched reviewed rule changes only the contract attached to the one parent arrow; a nonmatch retains the parent arrow and contract.
- [ ] Continue context research in separate immutable passes rather than repeatedly retuning v1: add policy/inflation state only as a separately declared context family; evaluate bounded two-context interactions only after one-dimensional evidence; accumulate prospective context matches; and revisit rejected/insufficient contexts only after the source archive materially expands.
