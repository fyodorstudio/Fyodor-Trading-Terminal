...
Updated 2026-09-13. The reopened P0–P7 implementation goal, post-P7 chart-switch stability fix, and Trade-review workflow are complete in code and automated validation. Owner visual gates remain listed below. The event-respect/high-TP campaign remains separately deferred and no research run or model mutation was performed. Rules: [AGENTS](../../AGENTS.md). Durable context: [CONTEXT](../../CONTEXT.md). Unknown owner: [navigation](../NAVIGATION.md).
...
- Completed Trade-review workflow: every dated Next setup has a Review action that changes to its pair, enables frozen arrows, selects only that registered setup, and opens its note row. Current/Recent rows expose Add/Edit note. Notes use the stable `market:pattern:event-time` identity, so a scheduled note follows the same event into Recent; they are stored in the separate `fms_review_notes` SQLite ledger and exposed at `GET /research/review-notes`, never written into immutable signals, assessments, contracts, or outcomes. Go to arrow now uses the configured Default refocus width. A validated last-known global FMS registry hydrates before React mounts and refreshes in parallel, so repeat cold starts can populate every registered pair immediately instead of briefly showing only EURUSD; quota failure falls back to a smaller markets-only acceleration snapshot and live bridge data remains authoritative.
- Latest reusable validation: 258/258 frontend checks, 93/93 bridge checks, TypeScript, production build, and diff hygiene pass. The five existing FastAPI/datetime deprecation warnings and the known large chart/FlagIcon chunk warning remain non-blocking; no browser automation was run.
...
- After one successful all-market load, fully close and reopen the frontend while the bridge is available. Trade > Next should restore all registered setup pairs immediately from the validated last-known snapshot, then update from the bridge without collapsing to an EURUSD-only intermediate list.
...
- The all-pair instant startup path needs one previously successful global-registry load to seed its browser snapshot. A genuinely first-ever launch still depends on the bridge response; local storage quota/corruption safely falls back to the live request rather than fabricating data.
